...
app.use(cookieParser()); // let me user cookie-parser
...
app.all('*', function (request, response, next) {
    ...
    referer = request.headers.referer; // IR1 A3 Erin Tachino // object of an HTTP request to store the URL of the page that linked to the current page
    if (referer) { // this is to ensure that if user is on a page that is from the server it will redirect them to last HTML file they visited
      const parsedReferer = url.parse(referer);
      if (parsedReferer.pathname.endsWith('.html')) { // parses the header 
...
// when called created session for cart
...
// creates the cart for the sesison if cart is valid 
app.get("/add_to_cart", function (request, response) {
    ...
    and set the submit_button parameter to the respective error message*/
        ...
        // Cited: Simple Shopping cart - server
...
app.post("/update_cart",function(request, response){
    new_qty = request.session.cart // var for session cart
    referrer = request.session.previousPage; // IR1 A3 Erin Tachino - send users to previous page 
    lastpage = referrer; // used so I can call lastpage globally 
    response.cookie('Lastpage', request.session.previousPage, {maxAge: 500000}); // sets him on how long cookie will last
    response.cookie('cart', new_qty, {
        expires: new Date(Date.now() + 3600000), // Expire in 1 hour
          });
    if(request.cookies.LogStatus == "true"){ // updates cart if user want to decrease or increase quantity 
        response.redirect('./cart.html?update=Cart Has Been Updated!')
    } else {
        response.redirect(lastpage); // if user is logged in then will sent user to cart.html since that was last visited page after hitting 'Update'
...
// writes after user clicks sent email 
app.get("/get_to_logout", function (request, response) {
    let LogStatus = false; // makes cookie of log in status false 
...
// write login when login button is clicked 
// Cited: Modifed Lab 15, ex5.js
// login page 
...
// login validation 
// cited: Modifed Lab 15 ex5.js and A2 login validation
app.post("/login", function (request, response) {
    ...
    referrer = request.session.previousPage; // IR1 A3 Erin Tachino - get session for previouspage set in app.all
    lastpage = referrer; // IR1 A3 Erin Tachino

    if (users[entered_email] != undefined) {
        let LogStatus = true; // lets cookie for logged in 
        ...
        // email, logged status, cart, and last page visited: Set with maxAge so expires 
        ...
        response.redirect(lastpage); // IR1 A3 sends user to previous HTML page 
    } else {
        response.send({ success: false, error: "Invalid username or password" });
    }
});

//  Cited: Modifed Lab15 ex5.js
...
app.post("/register", function (request, response) {
    ...
    if (Object.keys(reg_error).length == 0) {
        ...
        user_data[email].num_loggedIn = 1; // IR4 A2 Daniel Lott 
        user_data[email].last_date_loggin = Date();
        LogStatus = true; // when user registers set logged in cookie to true 
        referrer = request.session.previousPage; // send user back to previous page IR1 A3 Erin Tachino
        lastpage = referrer; // send user back to previous page IR1 A3 Erin Tachino

        /* creates a string using are variable fname which is from users and then JSON will stringify the data "users" 
        gets cookies for email, loggedin status, cart and lastpage */
        ...
        response.cookie('Lastpage', request.session.previousPage, {maxAge: 500000});
        response.redirect(lastpage); // IR1 A3 Erin Tachino
    } else {
...
// check out sets loggedin status 
